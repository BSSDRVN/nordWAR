...
mabl_fielding_url <- "https://www.tcmabl.com/teams/default.asp?p=stats&div=120467&viewseas=Summer_2025&cat=Fielding&u=TCMABL&s=baseball&t=print"

#Pitching stats URL for the Sunday league
mabl_pitching_url <- "https://www.tcmabl.com/teams/default.asp?p=stats&div=120467&viewseas=Summer_2025&cat=Pitching&u=TCMABL&s=baseball&t=print"
...
mabl_total_games <- 22
mabl_teams <- 16
mabl_opponent_name <- "Sea Donkeys"

mabl_total_season_wins_overall <- ((mabl_games_played * mabl_teams) /2)

nmtbl_games_played <- 14
nmtbl_total_games <- 14
nmtbl_teams <- 12
nmtbl_opponent_name <- "Minneapolis North Stars"
...
mabl <- mabl %>%
  mutate(M_replacement_runs = mabl_RPA
           #mabl_replacement_per_pa 
         * PA)



mabl_replacement_runs <- 10*(mabl_total_games/162)*((mabl_games_played)/mabl_total_games)
...
mabl_runs_per_win <- 
  2 * (sum(mabl$R) / (mabl_teams * mabl_games_played))
...
nmtbl <- nmtbl %>%
  mutate(M_replacement_runs = nmtbl_RPA
         * PA)

nmtbl_replacement_runs <- #0 
10*(nmtbl_total_games/162)*((nmtbl_games_played)/nmtbl_total_games)
#Note: This 10 was formerly 20 to match the below note.
#20 runs in 162 games is the per-player replacement level, so we scale back to however many games we have in a season
nmtbl_runs_per_win <- 
...
mabl_pitch <- mabl_pitch %>%
  mutate(FIP = ((13*HR)+(3*(BB+HBP))-(2*SO))/INN + mabl_FIPc,
         RA9 = R / (INN/9)) %>%
  filter(is.finite(FIP), INN > 0) #No infinite FIPs. I hate that.
...
mabl_war <- mabl_war %>%
  mutate(nordWAR = ((`wRAA (Batting Runs)` + wSB + fielding_runs +
         ...
         WPG = nordWAR / G,
         WvRep = (
      (`wRAA (Batting Runs)` + wSB + fielding_runs + PosAdj) - (mabl_RPA * PA)
    ) / mabl_runs_per_win,
...
nmtbl_war <- nmtbl_war %>%
  mutate(nordWAR = ((`wRAA (Batting Runs)` + wSB + fielding_runs +
         ...
         WPG = nordWAR / G,
         WvRep = (
      (`wRAA (Batting Runs)` + wSB + fielding_runs + PosAdj) - (nmtbl_RPA * PA)
    ) / nmtbl_runs_per_win,

    WvA = (
      ((`wRAA (Batting Runs)` + wSB + fielding_runs + PosAdj)
    ) / nmtbl_runs_per_win) + pitchWAR,
...
# Combine stats, since averaging averages never goes well
both_leagues <- both_leagues %>%
  mutate(
    ...
    AB = (AB_mabl + AB_nmtbl),
    HR = (HR_mabl + HR_nmtbl),
    HBP = (HBP_mabl + HBP_nmtbl),
    `1B` = (`1B_mabl` + `1B_nmtbl`),
    `2B` = (`2B_mabl` + `2B_nmtbl`),
    `3B` = (`3B_mabl` + `3B_nmtbl`),
    SacF = (SacF_mabl + SacF_nmtbl),
    AVG = H/AB,
    ...
    #OPS = OBP + SLG
    OPS = OBP + SLG,
    total_nordWAR = (nordWAR_mabl + nordWAR_nmtbl),
    WPG = (total_nordWAR / G)
    ) %>%
  select(No, Name, Team, AVG, OBP, SLG, OPS, G, 
         #G_mabl, G_nmtbl, 
         MABL = nordWAR_mabl, NMTBL = nordWAR_nmtbl, 
         nordWAR = total_nordWAR, WPG,
         #OBP/OPS/SLG diagnostics
         #AB, `1B`, `2B`, `3B`, HR
  )
...
# League FIP constant is just averaging the two others. This is terrible math, but it kind of has to be done, and here's why:
# In the state the data is currently in, I can't really get a two-league average of all the stats. Just one at a time.
# Probably what I could do is scale this by amount of innings pitched in each league:
lgFIPc <- (nmtbl_FIPc * (sum(both_pitching$INN_nmtbl)/(sum(both_pitching$INN_mabl, both_pitching$INN_nmtbl))))+
          (mabl_FIPc * (sum(both_pitching$INN_mabl)/(sum(both_pitching$INN_mabl, both_pitching$INN_nmtbl))))

both_pitching_after <- both_pitching %>%
  mutate(
    IP = (INN_mabl + INN_nmtbl),
    ER = (ER_mabl + ER_nmtbl),
    G = G_mabl + G_nmtbl,
    GS = GS_mabl + GS_nmtbl,
    W = W_mabl + W_nmtbl,
    ...
    HBP = HBP_mabl + HBP_nmtbl,
    K = SO_mabl + SO_nmtbl,
    total_pitchWAR = (pitchWAR_mabl + pitchWAR_nmtbl),
    # ERA = 9 x ER / IP
    ERA = ((9 * ER) / IP), #It is debatable whether this should be 7 for these leagues.
    ERAvg = ((ERA_mabl + ERA_nmtbl) / 2), #Is it the same? We will find out.
    
    # FIP = ((13*HR)+(3*(BB+HBP))-(2*K))/IP + constant    
    FIP = (((13*HR)+(3*(BB+HBP))-(2*K))/IP + lgFIPc), 
    WHIP = (BB + H)/(IP),
    pWIP = total_pitchWAR / IP
  ) %>%
  select(No, Name, Team, G, GS, IP, W, L, ERA, FIP, `WH/IP` = WHIP, MABL = pitchWAR_mabl, NMTBL = pitchWAR_nmtbl, pWAR = total_pitchWAR, `pW/IP` = pWIP)

# Only Babycakes pitchers
pitching_two <- both_pitching_after %>%
  filter(Team == "Bloomington Babycakes") %>%
  mutate(Team = "Babycakes") %>%
  select(-Team) %>%
  arrange(desc(pWAR))
