...
mabl_fielding_url <- "https://www.tcmabl.com/teams/default.asp?p=stats&div=120467&viewseas=Summer_2026&cat=Fielding&u=TCMABL&s=baseball&t=print"

#Pitching stats URL for the Sunday league
mabl_pitching_url <- "https://www.tcmabl.com/teams/default.asp?p=stats&div=120467&viewseas=Summer_2026&cat=Pitching&u=TCMABL&s=baseball&t=print"
...
nmtbl_fielding_url <- "https://www.nmtbl.com/teams/default.asp?p=stats&viewseas=Summer_2026&cat=Fielding&u=NMTBL&s=baseball&t=print"

#Pitching stats URL for the Thursday league
nmtbl_pitching_url <- "https://www.nmtbl.com/teams/default.asp?p=stats&viewseas=Summer_2026&cat=Pitching&u=NMTBL&s=baseball&t=print"
...
#This is fundamentally "How many games have been played across the league".

nmtbl_games_played <- 6 #This is down one for us until we make up the Stars game. I'm counting North Stars even though the stats aren't in.
nmtbl_total_games <- 15
nmtbl_teams <- 16
nmtbl_opponent_name <- "Tri City Twins"

nmtbl_total_season_wins_overall <- ((nmtbl_games_played * nmtbl_teams) /2)

wobascale <- 1.232 #using 2025 final stats
...
mabl <- mabl_scrape %>% 
  ...
  mutate(
    ...
    across(.cols = c(AVG, OBP, SLG, OPS), .fns = as.double)
  ) %>%
...
mabl_pitch <- mabl_scrape_pitching %>%
  janitor::row_to_names(row_number = 1) %>%
  mutate(
    across(.cols = c(G, GS, CG, SHO, W, L, SV, SVO, BF, H, R, ER, HR, HBP, BB,# IBB, 
                     SO, SacF, #SacB, BK, #which will come back whenever someone records one
                     B, S, 
                     PC, WP), .fns = as.integer),
...
nmtbl <- nmtbl_scrape %>% 
  janitor::row_to_names(row_number = 1) %>%
  mutate(
    across(.cols = c(G, PA, AB, R, H, `2B`, `3B`, HR, RBI, HBP, BB, SO, SacB, 
                     SacF, SB, CS), .fns = as.integer),
    across(.cols = c(AVG, OBP, SLG, #TB, 
                     OPS), .fns = as.double) #the thursday league has OPS and total bases for some reason
  ) %>% replace(is.na(.), 0) %>%
...
nmtbl_pitch <- nmtbl_scrape_pitching %>%
  janitor::row_to_names(row_number = 1) %>%
  mutate(
    across(.cols = c(G, GS, CG, SHO, W, L, SV, SVO, BF, H, R, ER, HR, HBP, BB, IBB, SO, SacF, SacB, 
                     BK, B, S, PC, WP, CI), .fns = as.integer),
    across(.cols = c(INN, WHIP, OBA, ERA), .fns = as.double)
  ) %>%
  replace(is.na(.), 0) %>% #all numeric values that should be 0 are now 0 and not NA
  filter(Name != "Overall Stats")
...
#all of our baserunners are simply okay, which is mostly true.


mabl_RPO <- sum(mabl$R) / (mabl_total_season_wins_overall * 21)
#I can use this because I am conservatively assuming everyone plays a complete game. 
#This is obviously false due to mercy rules, but it shouldn't break a whole ton of stuff.

mabl_runCS <- -(2 * mabl_RPO + 0.075) #How bad does it affect our runs to be caught stealing? (FG says -(2 x RunsPerOut + 0.075).)
nmtbl_RPO <- sum(nmtbl$R) / (nmtbl_total_season_wins_overall * 21)
nmtbl_runCS <- -(2 * nmtbl_RPO + 0.075)
runSB <- .2 #How well does it affect our runs to steal a base? (FG says set this at 0.2.)


mabl_lgwSB <- ((sum(mabl$SB) * runSB) + (sum(mabl$CS) * mabl_runCS)) / 
                   (sum(mabl$`1B`) + sum(mabl$BB) + sum(mabl$HBP))
#League stolen base runs (lgwSB): (SB * runSB + CS * runCS) / (1B + BB + HBP – IBB)

mabl <- mabl %>%
  mutate(wSB = (SB * runSB) + (CS * mabl_runCS) - (mabl_lgwSB * (`1B` + BB + HBP)))




nmtbl_lgwSB <- ((sum(nmtbl$SB) * runSB) + (sum(nmtbl$CS) * nmtbl_runCS)) / 
                   (sum(nmtbl$`1B`) + sum(nmtbl$BB) + sum(nmtbl$HBP))
#League stolen base runs (lgwSB): (SB * runSB + CS * runCS) / (1B + BB + HBP – IBB)

nmtbl <- nmtbl %>%
  mutate(wSB = (SB * runSB) + (CS * nmtbl_runCS) - (nmtbl_lgwSB * (`1B` + BB + HBP)))
...
mabl_field <- mabl_field %>%
  filter(TC>0) #Have you touched a baseball this year
...
#The 0.8 here is a calibration constant - it just scales down the relative impact of fielding
#on a player's NordWAR. Since we can't get good data, I'd hate for a guy who bats well to be 
#fighting an error committed during a blowout in, like, one or two fielding chances.

#It also continues to encourage 1.000s - essentially "Fielding well still saves runs".
...
mabl_pitch <- mabl_pitch %>%
  mutate(FIP = ((13*HR)+(3*(BB+HBP))-(2*SO))/INN + mabl_FIPc,
         RA9 = R / (INN/9)) %>%
  filter(is.finite(FIP)) #No infinite FIPs. I hate that.
