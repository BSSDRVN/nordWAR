...
fangraphs_guts_url <- "https://www.fangraphs.com/guts.aspx?type=cn"
```
...
nmtbl_teams <- 12
nmtbl_opponent_name <- "Legends"

nmtbl_total_season_wins_overall <- ((nmtbl_games_played * nmtbl_teams) /2)

park_factor <- 1
wobascale <- fg_scrape[1, ]$wOBAScale #first row's wobascale column
...
nmtbl <- nmtbl %>%
  mutate(
    ...
    wOBA = (fg_scrape[1, ]$wBB*BB +
              fg_scrape[1, ]$wHBP*HBP +
              fg_scrape[1, ]$w1B*`1B` +
              fg_scrape[1, ]$w2B*`2B` +
              fg_scrape[1, ]$w3B*`3B` +
              fg_scrape[1, ]$wHR*HR) / #all of this, over...
      (PA + BB + SacF + HBP)
  )
